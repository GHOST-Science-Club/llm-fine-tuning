# Bielik Supervised Fine-Tuning — GPU (HPC Eagle/Proxima)
...
> A TPU variant of this pipeline lives in [`../tpu/`](../tpu/README.md).
...
Copy `.env.example` to `.env` in the `supervised-fine-tuning/gpu` directory and fill in all values:
...
```bash
scp -i ~/.ssh/id_YOUR_ID -r ./gpu/* username@eagle.man.poznan.pl:/mnt/storage_6/project_data/YOUR_GRANT/supervised-fine-tuning/gpu
```
...
```bash
cd /mnt/storage_6/project_data/YOUR_GRANT/supervised-fine-tuning/gpu
bash setup.sh
```
...
* **`ERROR: Set GRANT in .env`:** The scripts can't find your project path. Make sure `.env` has `GRANT=your_grant_id` and that you run `sbatch`/`bash setup.sh` from the `supervised-fine-tuning/gpu` directory.
